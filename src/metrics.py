...
def print_metrics(inputs, outputs, corrections, dataset, ocr_prediction):
    ...
    cer_corr = np.mean([get_cer(gt, corr) for gt, corr in zip(outputs, corrections)])
    wer_corr = np.mean([get_wer(gt, corr) for gt, corr in zip(outputs, corrections)])
    
    print(f"{dataset}, {ocr_prediction}, {(cer_base * 100):.2f}%, {(wer_base * 100):.2f}%, {(cer_corr * 100):.2f}%, {(wer_corr * 100):.2f}%")
